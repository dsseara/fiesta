...
netCurl.pre = sum(sum(curl(binCentersX.pre, binCentersY.pre, fluxField.pre(:,:,1), fluxField.pre(:,:,2))))*dx.pre*dy.pre;
...
quiver(binCentersX.pre, binCentersY.pre, fluxField.pre(:,:,1), fluxField.pre(:,:,2),'w', 'LineWidth', 1.5);
xlabel('\Deltax (\mum)')
ylabel('\Deltay (\mum)')
title(['Active systems, net curl=', num2str(netCurl.pre)])

% Do the control on a system without myosin
path = '/Volumes/Storage/Danny/122311_5ulRED_2pt0uMact_phall_beads_nta_nochol_mc_1to300xlink_2/tc1_5s/msd/';
nbins = 25;
dt = 5;
rg_cutoff = [0,Inf];
tc = []; 
